...
class _BodyState extends State<Body> {
  ...
  void initState() {
    super.initState();
    initPlatformState();
    //_appKey = new provisioning.AppKey() (name, 27dJuNwLbUW_ujSJ, _keyStr) "mAHnpg.uiHY7g:27dJuNwLbUW_ujSJ";
  }
  ...
  void createAblyRest() async {
    ...
    final clientOptions =
        ably.ClientOptions.fromKey("mAHnpg.uiHY7g:27dJuNwLbUW_ujSJ");
  ...
  void createAblyRealtime() async {
    ...
    debugPrint("App key: ${_appKey.toString()}");
    final clientOptions =
        ably.ClientOptions.fromKey("mAHnpg.uiHY7g:27dJuNwLbUW_ujSJ");
    //clientOptions.environment = 'realtime';
    clientOptions.logLevel = ably.LogLevel.verbose;
    clientOptions.logHandler = ({String msg, ably.AblyException exception}) {
      print("Custom logger :: $msg $exception");
    };
    ...
    try {
      ...
      ably.RealtimeChannel channel = realtime.channels.get("timeline");
  ...
  listenRealtimeConnection(ably.Realtime realtime) async {
    var alphaSubscription = realtime.connection
        .on()
        .listen((ably.ConnectionStateChange stateChange) async {
      print(
          '${DateTime.now()} : ConnectionStateChange event: ${stateChange.event}'
  ...
  listenRealtimeChannel(ably.RealtimeChannel channel) async {
    var _channelStateChangeSubscription =
  ...
  static Color opStateColor(OpState state) {
    switch (state) {
      case OpState.NotStarted:
        return Color.fromARGB(255, 192, 192, 255);
      case OpState.InProgress:
  ...
  static Widget button(
      ...
      FlatButton(
        ...
        child: Text(
          opStateDescription(
              state, actionDescription, operatingDescription, doneDescription),
        ),
        ...
        onPressed: (_realtimeConnectionState == ably.ConnectionState.connected)
            ? () async {
                ably.RealtimeChannel channel =
                    _realtime.channels.get("timeline");
                print(
                    "Attaching to channel ${channel.name}: Current state ${channel.state}");
                try {
        ...
        onPressed: (_realtimeChannelState == ably.ChannelState.attached)
            ? () {
                ably.RealtimeChannel channel =
                    _realtime.channels.get("timeline");
                print(
                    "Detaching from channel ${channel.name}: Current state ${channel.state}");
                channel.detach();
                print("Detached");
              }
            : null,
  ...
  Widget createChannelSubscribeButton() => FlatButton(
        onPressed: (_realtimeChannelState == ably.ChannelState.attached &&
            ...
            ? () {
                ably.RealtimeChannel channel =
                    _realtime.channels.get("timeline");
                Stream<ably.Message> messageStream =
                    channel.subscribe(names: ['timeline.update']);
  ...
  Widget createChannelPublishButton() => FlatButton(
        onPressed: (_realtimeChannelState == ably.ChannelState.attached)
            ? () async {
                ...
                dynamic data = messagesToPublish[
                    (realtimePublishCounter++ % messagesToPublish.length)];
                ably.Message m = ably.Message()
                  ..data = data
                  ..name = 'Hello';
                try {
        ...
        color: Colors.yellow,
        child: Text(
            'Publish: ${messagesToPublish[(realtimePublishCounter % messagesToPublish.length)]}'),
      );
        ...
        body: Center(
          child: ListView(
              ...
              children: [
                ...
                Text('App Key: ' +
                    ((_appKey == null)
                        ? 'Ably not provisioned yet.'
                        : _appKey.toString())),
                Divider(),
                createRealtimeButton(),
                Text('Realtime: ' +
                    ((_realtime == null)
                        ? 'Ably Realtime not created yet.'
                        : _realtime.toString())),
                ...
                Text(
                    'Message from channel: ${((channelMessage == null) ? '-' : channelMessage.data)}'),
                createChannelPublishButton(),
                Divider(),
                createRestButton(),
                Text(
                    'Rest: ${((_rest == null) ? 'Ably Rest not created yet.' : _rest.toString())}'),
                sendRestMessage(),
                Text(
                    'Rest: press this button to publish a new message with data "Flutter ${msgCounter}"'),
              ]),
        ),
      ),
    );
